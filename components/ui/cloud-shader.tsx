...
export type CloudShaderProps = {
  ...
  cloudColor?: string;
  /** Sky color at the very top of the gradient. */
  skyTopColor?: string;
  /** Sky color at the midpoint of the gradient. */
  skyMidColor?: string;
  /** Sky color at the very bottom of the gradient. */
...
void main() {
  ...
  vec3 sky = v_uv.y < 0.5
    ? mix(u_skyBottom, u_skyMid, v_uv.y * 2.0)
    : mix(u_skyMid, u_skyTop, (v_uv.y - 0.5) * 2.0);
  vec3 color = sky;
...
export const CloudShader = ({
  ...
  count = 6,
  cloudColor = "#fbf8f2",
  skyTopColor = "var(--primary)",
  skyMidColor = "#3876ba",
  skyBottomColor = "var(--primary)",
}: CloudShaderProps) => {
  ...
  useEffect(() => {
    ...
    const loc = {
      ...
      skyMid: gl.getUniformLocation(program, "u_skyMid"),
    ...
    const draw = (now: number) => {
      ...
      const skyMid = parseHex(p.skyMidColor);
      ...
      gl.uniform3f(loc.skyMid, skyMid[0], skyMid[1], skyMid[2]);
