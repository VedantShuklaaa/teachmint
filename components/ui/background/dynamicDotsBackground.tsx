...
const DotField = memo(({
	...
	useEffect(() => {
		...
		function tick() {
			...
			if (glowEl) {
				glowEl.style.transform = `translate(${m.x - glowSize / 2}px, ${m.y - glowSize / 2}px)`;
				glowEl.style.opacity = String(glowOpacity.current);
			}

			ctx!.clearRect(0, 0, w, h);

			ctx!.fillStyle = p.backgroundColor as string;
			...
			ctx!.fillStyle = p.dotColor as string;
			ctx!.globalAlpha = p.dotOpacity as number;
			ctx!.beginPath();
